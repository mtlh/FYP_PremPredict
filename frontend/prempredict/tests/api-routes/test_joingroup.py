...
class JoinGroupApiEndpoint(TestCase):
    ...
    def test_url_exists_at_correct_location(self):
        response = self.client.post("/leaderboard/join")
        self.assertEqual(response.status_code, 200)

    def test_url_available_by_name(self):  
        response = self.client.post(reverse("Join A Custom Leaderboard"))
        self.assertEqual(response.status_code, 200)
    ...
    def test_endpoint_sucess_case(self):
        ...
        self.assertEqual(response.content.decode('utf-8'), 'Joined Group!')
    ...
    def test_endpoint_incorrect_inv(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIsInstance(response.content, bytes)
        self.assertEqual(response.content.decode('utf-8'), 'Group does not exist.')
